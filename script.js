document.addEventListener('DOMContentLoaded', function() {
    ...
    const translations = {
        ...
        "One bright and sunny morning, I decided to go on an adventure.": {
            translation: "Нэгэн <strong>гэгээлэг</strong>, нартай өглөө би адал явдалд гарахаар шийдсэн.",
    ...
    textElement.addEventListener('click', handleEvent);
    textElement.addEventListener('touchstart', handleEvent);

    function handleEvent(event) {
        const sentence = getSentenceUnderCursor(event);
        console.log('Clicked sentence:', sentence);  // Debugging line
        if (sentence && translations[sentence]) {
            const data = translations[sentence];
            ...
            console.error('Translation not found for sentence:', sentence);
        }
    }
